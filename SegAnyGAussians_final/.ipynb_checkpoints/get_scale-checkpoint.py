...
if __name__ == '__main__':
    ...
    for it, view in tqdm(enumerate(cameras)):
        ...
        # Check if the mask is valid 2D
        if corresponding_masks.ndim != 2:
            print(f"[WARNING] Skipping image {viewpoint.image_name} because mask shape is {corresponding_masks.shape}")
            continue
        
        upsampled_mask = torch.nn.functional.interpolate(
            corresponding_masks.unsqueeze(0).unsqueeze(0),  # Add (N=1, C=1) dimensions
            mode='bilinear',
            size=(depth.shape[0], depth.shape[1]),
            align_corners=False
        ).squeeze(0).squeeze(0)  # remove extra dimensions after interpolate
