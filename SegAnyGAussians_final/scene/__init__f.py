...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel=None, feature_gaussians: FeatureGaussianModel=None, load_iteration=None, feature_load_iteration=None, shuffle=True, resolution_scales=[1.0], init_from_3dgs_pcd=False, target='scene', mode='train', sample_rate = 1.0, start_idx=0, end_idx=None):
        """b
        ...
        # and not os.path.exists(os.path.join(args.source_path, "transforms.json")):
            ...
            scene_info = sceneLoadTypeCallbacks["Colmap"](args.source_path, args.images, args.eval, need_features = args.need_features, need_masks = args.need_masks, sample_rate = sample_rate, allow_principle_point_shift = args.allow_principle_point_shift, replica = 'replica' in args.model_path,start_idx=start_idx,end_idx=end_idx) #niranajna
        ...
        for resolution_scale in resolution_scales:
            ...
            if start_idx is not None or end_idx is not None:
                partial_train_cameras = scene_info.train_cameras[start_idx:end_idx]
            else:
                partial_train_cameras = scene_info.train_cameras
            self.train_cameras[resolution_scale] = cameraList_from_camInfos(partial_train_cameras, resolution_scale, args)
            
            print("Loading Test Cameras")
            if start_idx is not None or end_idx is not None:
                partial_test_cameras = scene_info.test_cameras[start_idx:end_idx]
            else:
                partial_test_cameras = scene_info.test_cameras
            self.test_cameras[resolution_scale] = cameraList_from_camInfos(partial_test_cameras, resolution_scale, args)
