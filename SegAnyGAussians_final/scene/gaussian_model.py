...
class GaussianModel:
    ...
    def load_ply(self, path):
        ...
        #self._opacity = nn.Parameter(torch.tensor(opacities, dtype=torch.float, device="cuda").requires_grad_(True))#niranjana added
        self._opacity = nn.Parameter(torch.tensor(np.ascontiguousarray(opacities), dtype=torch.float, device="cuda").requires_grad_(True))
    ...
    def segment(self, mask=None):
        assert mask is not None
            # mask = (self._mask > 0)
        #mask = mask.squeeze() #niranjana got error while rendering 
        mask = (mask > 0.5).to(dtype=torch.bool, device=self._xyz.device)
