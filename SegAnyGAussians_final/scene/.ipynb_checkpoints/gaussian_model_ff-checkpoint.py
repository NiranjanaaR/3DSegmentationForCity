...
class FeatureGaussianModel(nn.Module):
    ...
    def load_ply_from_3dgs(self, path):
        ...
        # self._point_features = nn.Parameter(torch.tensor(features_extra, dtype=torch.float, device="cuda").contiguous().requires_grad_(True))
        #self._opacity = nn.Parameter(torch.tensor(opacities, dtype=torch.float, device="cuda").requires_grad_(True))
        self._opacity = nn.Parameter(torch.tensor(np.ascontiguousarray(opacities), dtype=torch.float, device="cuda").requires_grad_(True)) #niranajna added
        self._scaling = nn.Parameter(torch.tensor(scales, dtype=torch.float, device="cuda").requires_grad_(True))
        self._rotation = nn.Parameter(torch.tensor(rots, dtype=torch.float, device="cuda").requires_grad_(True))
    ...
    #added by niranjana for parallel processing 
    def forward(self, viewpoint_cam, pipe, background, norm_point_features=True, smooth_type='traditional', smooth_weights=None, smooth_K=16):        
        from gaussian_renderer import render_contrastive_feature
        return render_contrastive_feature(
            viewpoint_cam,
            self,
            pipe,
            background,
